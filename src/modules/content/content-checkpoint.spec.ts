...
describe('ContentService - Lesson Checkpoints', () => {
  ...
  function MockCheckpointAnswerModel(this: any, data: any) { this.save = jest.fn().mockResolvedValue({ _id: 'ans1', ...data }); }
  ...
  it('creates a lesson checkpoint and returns saved checkpoint', async () => {
    ...
    const dto = { questionText: 'What is 1+1?', timestampSeconds: 10, type: 'MCQ', options: [{ text: '2', isCorrect: true }] };

    const res = await service.createLessonCheckpoint('org1', 'course1', 'lesson1', dto);

    expect(mockLessonModel.findOne).toHaveBeenCalledWith({ _id: 'lesson1', organizationId: 'org1', courseId: 'course1', isDeleted: false });
    expect(mockCheckpointModel).toHaveBeenCalled();
    expect(res).toMatchObject({ questionText: dto.questionText, timestampSeconds: dto.timestampSeconds });
  });
  ...
  it('returns checkpoints list for a lesson', async () => {
    ...
    const res = await service.getLessonCheckpoints('org1', 'course1', 'lesson1');
    expect(mockCheckpointModel.find).toHaveBeenCalledWith({ organizationId: 'org1', courseId: 'course1', lessonId: 'lesson1', isDeleted: false });
  ...
  it('answers a checkpoint and marks progress when correct', async () => {
    ...
    const res = await service.answerLessonCheckpoint('org1', 'student1', 'course1', 'lesson1', 'ck1', dto);

    expect(mockCheckpointModel.findOne).toHaveBeenCalledWith({ _id: 'ck1', organizationId: 'org1', lessonId: 'lesson1', courseId: 'course1', isDeleted: false });
    expect(mockProgressModel.findOneAndUpdate).toHaveBeenCalled();
    expect(mockProgressModel.updateOne).toHaveBeenCalledWith({ _id: 'prog1' }, { $addToSet: { checkpointPassedIds: checkpoint._id } });
