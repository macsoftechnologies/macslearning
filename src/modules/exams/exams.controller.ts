...
import { CreateExamDto, GradeShortAnswerDto, QuestionDto, SaveAnswerDto, SubmitAttemptDto, UpdateQuestionDto } from './dto/exams.dto';
...
export class ExamsController {
  ...
  async getQuestions(@Request() req: any, @Param('examId') examId: string) {
    ...
    return this.examsService.getQuestions(req.user.organizationId, examId, req.user.userType);
  }

  @Patch('exams/:examId/answers')
  async saveAnswer(@Request() req: any, @Param('examId') examId: string, @Body() answerDto: SaveAnswerDto) {
    if (req.user.userType === 'STUDENT') {
      const exam = await this.examsService.getExamById(req.user.organizationId, examId);
      await this.enrollmentService.verifyActiveEnrollment(req.user.organizationId, req.user.userId, exam.courseId.toString());
    }
    return this.examsService.saveAnswer(req.user.organizationId, req.user.userId, examId, answerDto);
  }
  ...
  async startAttempt(@Request() req: any, @Param('examId') examId: string) {
    ...
    return this.examsService.startAttempt(req.user.organizationId, req.user.userId, examId);
  }

  @Get('exams/:examId/attempts')
  async getAttemptHistory(@Request() req: any, @Param('examId') examId: string) {
    if (req.user.userType === 'STUDENT') {
      const exam = await this.examsService.getExamById(req.user.organizationId, examId);
      await this.enrollmentService.verifyActiveEnrollment(req.user.organizationId, req.user.userId, exam.courseId.toString());
    }
  ...
  async submitAttempt(@Request() req: any, @Param('examId') examId: string, @Body() submitDto: SubmitAttemptDto) {
    if (req.user.userType === 'STUDENT') {
      const exam = await this.examsService.getExamById(req.user.organizationId, examId);
      await this.enrollmentService.verifyActiveEnrollment(req.user.organizationId, req.user.userId, exam.courseId.toString());
    }
    return this.examsService.submitAttempt(req.user.organizationId, req.user.userId, examId, submitDto.answers);
  }
  ...
  async getMyResult(@Request() req: any, @Param('examId') examId: string) {
    ...
    return this.examsService.getMyResult(req.user.organizationId, req.user.userId, examId);
  }
